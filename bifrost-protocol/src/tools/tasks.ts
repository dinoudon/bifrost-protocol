...
export function availableTasks(db: Database, agentSkills: string[]) {
  // BOLT ⚡: Offload JSON skill filtering to SQLite for better performance
  // Uses json_each to check if any of the agent's skills match the task's required skills
  return db.prepare(`
    SELECT * FROM tasks
